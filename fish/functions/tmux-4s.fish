function tmux-4s -d "Starts tmux with 4 windows for a project and its Python environment"
    if test (count $argv) -ne 2
        echo "Usage: tmux-4s <project_name> <python_environment>"
        return 1
    end
...
for f in ~/Developer/e/*
    if test -d $f
        ...
        complete -c tmux-4s -n "test (count (commandline -opc)) -eq 2" -a (basename $f) -d "Python $envver"
    end
end
